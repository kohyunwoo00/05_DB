...
CREATE USER TEST01_USER IDENTIFIED BY "TEST1234";

/* 시스템 권한 부여
 * - GRANT 권한, .... TO 사용자명;
 * */

GRANT CREATE SESSION TO TEST01_USER;
-- 연결 확인됨

/* 생성된 사용자 계정으로 접속(TEST01_USER) */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY, TEST_COL VARCHAR2(30) NOT NULL
);
--> 권한이 불충분합니다
-- + 사용 가능한 공간(TABLESPACE) 할당

/* 다시 관리자로 접속 */
GRANT RESOURCE TO TEST01_USER;
...
/* TEST01_USER에게 테이블 생성 공간 할당 
 * - USERSㅠㅗㄹ더 내에 10M 할당
 * */
ALTER USER TEST01_USER
DEFAULT TABLESPACE USERS
QUOTA 10M ON USERS;

/* 다시 TEST01_USER 계정 접속 후 테이블 생성 */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY, TEST_COL VARCHAR2(30) NOT NULL
);
--> 생성 성공
...
/* 객체 권한 부여 */
SELECT * 
FROM KH01_KHW.EMPLOYEEL; --> 없어서 조회 X
