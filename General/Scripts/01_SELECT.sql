 ...
 * 	 조회 결과(RESULT SET)에서 모든 행에 산술 연산이 적용된 컬럼 값이 조회된다 
 */
 ...
 * 
 * - TIMESTAMP : 년,월,일,시,분,초,요일,ms,지역  저장
 * 
 * 
 * - SYS (시스템) : 시스템에 설정된 시간 기반
 * - CURRENT : 현재 접속한 사용자의 시간 기반
 * 
 * - SYSDATE : 현재 시스템 시간 조회
...
SELECT EMP_ID, EMP_NAME, HIRE_DATE, FLOOR(CURRENT_DATE - HIRE_DATE), --내림 처리(소수점 지우기)   
...
-- 단, 컬럼명은 모두 별칭 적용
SELECT EMP_ID AS 사번,EMP_NAME 이름, SALARY AS "급여(원)", SALARY * 12 "연봉(급여 *12)"
FROM EMPLOYEE; 
---------------------------------------------------------------------------------

/* 연결 연산자(||)
 * - 두 컬럼값, 리터럴을 하나의 문자열로 연결할 때 사용
 *  */
 ...
 *   중복을 제거하고 한 번만 표시할 때 사용(중복된 데이터를 가진 행을 제거)
 */

-- EMPLOYEE 테이블에서 모든 사원의 부서 코드(DEPT_CODE) 조회
SELECT DEPT_CODE
FROM EMPLOYEE; -- 23행 조회(중복 O)

-- 사원들이 속한 부서 코드 조회
--> 사원이 있는 부서만 조회
SELECT DISTINCT DEPT_CODE
FROM EMPLOYEE; -- 7행 조회(중복 X)
...
/* *** WHERE 절 ***
 * - 테이블에서 조건을 충족하는 행을조회할 때 사용
 * 
 * - WHERE 절에는 조건식(결과가 T/F)만 작성 가능
 * 
 * - 비교 연산자 : >, <, >=, <=, =(같다), !=, <>(같지 않다)
 * 
 * - 논리 연산자 : AND, OR, NOT  -- 영어 단어로 쓰기
 * */

-- EMPOLYEE 테이블에서 급여가 400만원을 초과하는 사원의
-- 사번, 이름, 급여를 조회
...
-- EMPOLYEE 테이블에서 급여가 500만원을 이하인 사원의
-- 사번, 이름, 급여, 부서코드, 직급코드를 조회
SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE, JOB_CODE 
FROM EMPLOYEE 
WHERE SALARY <= 5000000;

-- EMPOLYEE 테이블에서 연봉이 5천만원을 이하인 사원의
-- 이름, 연봉 조회
 ...
 * 
 * - AND(그리고) : 두 연산의 결과가 TRUE 일때만 최종 결과 TRUE
 * -> 두 조건을 모두 만족하는 행만 결과 집합(RESULT SET)에 포함
 * 
 * - OR(또는) : 두 연산의 겨로가가 FLASE 일떄만 최종 결과 FALSE
 * -> 두 조건 중 하나라도 만족하는 행을 결과 집합(RESULT SET)에 포함
 * 
 * - 우선 순위 : AND > OR
 * */

-- EMPLOYEE 테이블에서 부서 코드가 'D6'인 사원 중
-- 급여가 400만원을 초과하는 사원의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D6'
AND SALARY > 4000000;

-- EMPLOYEE 테이블에서 급여가 300만 이상, 500만 미만의 사원의
-- 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY 
FROM EMPLOYEE 
...
SELECT EMP_NAME 
FROM EMPLOYEE 
WHERE EMP_NAME LIKE '%하%'; -- 정중하,하이유,하동윤,유하진
...
SELECT EMP_ID, EMP_NAME, EMAIL 
FROM EMPLOYEE 
WHERE EMAIL LIKE '___#_%' ESCAPE '#'; --12행 #자리에 특수문자 아무거나 하나 넣어도 됨
 ...
 *       [ASC / DESC]  (오름차순 / 내림차순)
 * 			 [NULLS FIRST / NULLSLAST] (NULL 데이터 위치 지정)
 * 
 * ** 중요 **
 * ORDER BY절은 해당 SELECT문 제일 마지막에만 수행
 * 
 * - 오름차순(ASCENDING) : 점점 커지는 순서로 정렬(ASC)
 * EX) 1 -> 10, A -> Z, 가 -> 하, 과거 -> 미래(날짜)
 * 
 * - 내림차순(DESCENDING) : 점점 작아지는 순서로 정렬(DESC)
 */

-- EMPLOYEE 테이블의 모든 사원을 이름 오름 차순으로 정렬하여 조회
SELECT EMP_NAME 
FROM EMPLOYEE 
...
/* 별칭을 이용하여 정렬하기 
 * - ORDER BY절은 제일마지막에 해석된다
 * 
 * - ORDER BY절 보다 먼저 해석되는 SELECT절 별칭을
 *   ORDER BY절에 인식할 수 있다.
 * 
...
-- ORDER BY절 해석 전 SELECT, FROM절 모두 해석되어 있기 때문에
