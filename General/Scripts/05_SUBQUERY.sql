...
-- 1. 단일행 서브쿼리 (SINGLE ROW SUBQUERY) = 단일행 
...
GROUP BY DEPT_CODE
HAVING SUM(SALARY) = (
	SELECT MAX(SUM(SALARY)) -- 서브 쿼리 -> 21760000
	FROM EMPLOYEE 
	GROUP BY DEPT_CODE
);
 ...
 * 그룹 함수 또는 GROUP BY에 사용된 컬럼명만 작성가능
 * */

-- 부서별 인원 수가 가장 적은 부서의
-- 부서명, 인원 수 조회
-- 단, 부서 이름이 없으면 '없음' 으로 표시
SELECT NVL(D.DEPT_TITLE, '없음') DEPT_TITLE, COUNT(*) 
FROM EMPLOYEE E
...
SELECT E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE,'없음') DEPT_TITLE 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE);
...
LEFT JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
WHERE E.EMP_ID NOT IN( -- 사수 역할을 갖지 않는 사원만 조회
	SELECT DISTINCT MANAGER_ID
	FROM EMPLOYEE 
	WHERE MANAGER_ID IS NOT NULL
);
...
SELECT E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE,'없음') DEPT_TITLE, J.JOB_CODE 
,CASE 
	-- 현재 행의 EMP_ID와 일치하는 값이 서브쿼리 결과에 있으면
	-- '사수' 없으면 '사원'
	WHEN E.EMP_ID IN(
...
FROM EMPLOYEE E
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
WHERE J.JOB_NAME = '대리' AND E.SALARY > ( -- 과장 최소 급여보다 많이 받는 대리 조회
	SELECT MIN(E.SALARY) 
	FROM EMPLOYEE E
	JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
	WHERE J.JOB_NAME = '과장'
...
FROM EMPLOYEE E
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
WHERE J.JOB_NAME = '대리' AND E.SALARY > ANY ( -- 과장 최소 급여보다 많이 받는 대리 조회
	SELECT E.SALARY 
	FROM EMPLOYEE E
	JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
	WHERE J.JOB_NAME = '과장'
...
SELECT E.EMP_ID, E.EMP_NAME, J.JOB_CODE , E.DEPT_CODE, D.DEPT_TITLE , J.JOB_NAME 
FROM EMPLOYEE E
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON(D.DEPT_ID = E.DEPT_CODE)
WHERE (E.DEPT_CODE,J.JOB_CODE) = (
	SELECT DEPT_CODE, JOB_CODE 
	FROM EMPLOYEE 
	WHERE EMP_NAME = '노옹철'
) AND E.EMP_NAME != '노옹철';
 ...
 * 3) (메인 쿼리 WHERE절 사용 시)서브쿼리의 결과가 먼저 해석된 메인 쿼리 1행에 영향을 줌 
 * -> 최종결과(RESULT SET)에 포함 될지 안될지를 지정
 * */

-- 직급별 급여 평균보다 급여를 많이 받는 직원의 
-- 이름, 직급코드, 급여 조회
SELECT  MAIN.EMP_ID, MAIN.JOB_CODE, MAIN.SALARY 
FROM EMPLOYEE MAIN
WHERE SALARY > (
...
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, NVL(D.DEPT_TITLE,'없음') DEPT_TITLE,
J.JOB_CODE, MAIN.HIRE_DATE 
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT D ON (MAIN.DEPT_CODE = D.DEPT_ID)
...
WHERE MAIN.HIRE_DATE = (
	...
	FROM EMPLOYEE SUB
	WHERE NVL(SUB.DEPT_CODE, '소속 없음') = NVL(MAIN.DEPT_CODE, '소속 없음') AND ENT_YN != 'Y'
)
...
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5' AND ENT_YN != 'Y';-- 퇴사자 제외
----------------------------------------------------------------------------------
-- 6. 스칼라 서브쿼리(= SELECT절에 사용되는 단일 행 서브쿼리)
...
SELECT NVL(E.DEPT_CODE,'없음') "부서 코드", NVL(D.DEPT_TITLE, '없음') "부서명", FLOOR(AVG(E.SALARY)) AS "급여 평균" 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
...
-- 3) 급여 평균이 3위 안에 드는 부서의 부서코드와 부서명, 평균급여를 조회
------------------------------------------------------------------------
-- 8. WITH
...
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 고용일
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, 
			E.MANAGER_ID , E.EMP_NO, E.HIRE_DATE 
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
WHERE (E.MANAGER_ID, E.DEPT_CODE) = (
...
-- 입사일이 빠른 순으로 조회하시오
-- 단, 퇴사한 직원은 제외하고 조회.. 
SELECT E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE ,'소속없음'), J.JOB_NAME,HIRE_DATE 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
JOIN JOB J ON(J.JOB_CODE = E.JOB_CODE)
WHERE HIRE_DATE = (
	SELECT MIN(HIRE_DATE)
	FROM EMPLOYEE 
	WHERE NVL(DEPT_CODE, '소속 없음') = NVL(E.DEPT_CODE, '소속 없음') AND ENT_YN != 'Y'
)
AND ENT_YN != 'Y'
...
FLOOR(
	MONTHS_BETWEEN(SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO,1,6), 'YYYYMMDD'))/12 
) AS "만 나이",
