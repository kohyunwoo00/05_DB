...
SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(BONUS, 0) "SALARY * BONUS" 
FROM EMPLOYEE; 

-- EMPLOYEE 테이블에서
-- 사번, 이름 전화번호를 조회
-- 단, 전화번호가 없으면 '없음', 전화번호가 있으면 '010********' 형식으로 변경해서 조회
SELECT EMP_ID, EMP_NAME, NVL2(PHONE,RPAD(SUBSTR(PHONE,1,3), LENGTH(PHONE), '*'), '없음') PHONE 
FROM EMPLOYEE;                         --SUBSTR(문자열, 시작위치, 길이)
...
/* 그룹 함수 
 * - N개 행의 컬럼 값을 함수로 전달하였을 때 결과가 1개만 반환되는 함수
 * 
...
-- 모든 사원의 급여 평균 조회
SELECT FLOOR(AVG(SALARY)) "급여 평균" 
FROM EMPLOYEE;

-- 모든 사원의 급여 평균 보다 많이 받는
-- 사원의 이름, 급여 조회 내림차순 조회
SELECT EMP_NAME 이름, SALARY 급여
FROM EMPLOYEE 
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) -- 서브 쿼리
-- 일반적으로 일반 컬럼 + 그룹함수 결과를 혼용 사용 불가 -> GROUP BY 또는서브쿼리
 ...
 * 
 * 1) COUNT(*)
 * - 조회된 모든 행의 개수를 반환(NULL이 포함 행도 개수 증가)
 * 2) COUNT(컬럼명)
 * - 지정된 컬럼 값이 NULL인 행을 제외하고 나머지 행의 개수 반환
 * 3) COUNT(DISTINCT 컬럼명)
 * - 컬럼 값이 중복되는 경우를 제외하고 행의 개수 반환(중복은 한 번만 카운트)
 * */
...
/* 방법 2 - COUNT(컬럼명) 사용 */
SELECT COUNT(PHONE) -- PHONE 컬럼 값이 NULL인 경우 제외하고 카운
FROM EMPLOYEE; 
