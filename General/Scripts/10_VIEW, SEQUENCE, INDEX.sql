...
CREATE VIEW V_EMP 
AS 
SELECT E.EMP_ID AS 사번, E.EMP_NAME AS 이름, NVL(D.DEPT_TITLE,'부서없음') AS 부서명,
J.JOB_NAME AS 직급명
FROM EMPLOYEE E
...
-- 시퀀스 테스트용 테이블 생성
CREATE TABLE TB_TEST(TEST_NO NUMBER PRIMARY KEY,
...
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 105 -> 5씩 증가 확인

-- TB_TST 테이블에 INSERT 시 PK 컬럼 값을 
-- 시퀀스로 얻어와서 삽입
...
-- SEQ_TEST_NO의 증가 값을 10, MAXVALUE를 200으로 수정
