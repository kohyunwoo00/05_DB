...
-- 2) DEPT_CODE가 같은 행을 그룹으로 묶음
-- 3) 묶은 그룹의 DEPT_CODE와 각 그룹에 속한 인원의 급여 합을 조회

-- EMPLOYEE 테이블에서 
-- 부서코드, 부서 별 급여의 합계, 부서 별 급여의 평균(정수처리), 인원 수를 조회하고 
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE, SUM(SALARY) "급여 합", FLOOR(AVG(SALARY)) 평균, COUNT(*) "인원 수"
FROM EMPLOYEE 
GROUP BY DEPT_CODE
...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE, COUNT(BONUS) "보너스 받는 사원 수" -- COUNT(컬럼명) : 컬럼 값이 NULL인 행 제외하고 카운트
FROM EMPLOYEE 
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
...
SELECT DEPT_CODE, FLOOR(AVG(SALARY)) "평균 급여" -- 4 그룹별 평균 조회
FROM EMPLOYEE -- 1 테이블 선택
...
FROM EMPLOYEE 
GROUP BY DEPT_CODE, JOB_CODE -- 1) 부서 그룹 2) 부서내 직급 그룹
ORDER BY DEPT_CODE ASC;
...
FROM EMPLOYEE --1
WHERE SALARY >= 3500000 --2
GROUP BY DEPT_CODE --3
ORDER BY DEPT_CODE ASC; --5
--> 개인 급여가 350만 이상인 사람들만 모아서 부서별로 그룹을 나누고 평균을 구함 -> 요구사항과 다름
...
--> 전체를 일단 그룹으로 만들어서 BONUS 받는 사람들을 카운트 하는 것이
-- 0이라는 결과가 조회하기 때문에 통계자료 측면에 더 좋음
